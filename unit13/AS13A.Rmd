...
### Ans.
1.當`OfficeVisits`和`Narcotic`分別等於他們的第一分位(`Q1`)時：<br>
(1).`PoorCare = 1`的機率<br>
```{r}
df = data.frame(
...
(2). 兩個自變數的勝率效果
```{r}
#因為係數的指數就是勝率比，因此將係數分別取exp()
c(exp(0.0821), exp(0.0763))
```
...
c(predict(glm1, df_1, type="response"), predict(glm1, df_2, type="response"))
```

2.當`OfficeVisits`和`Narcotic`分別等於他們的第三分位(`Q3`)時：<br>
(1). `PoorCare = 1`的機率<br>
```{r}
df = data.frame(
...
(2). 兩個自變數的勝率效果<br>
（模型setting未改變，所以同一模型下勝率效果會與上題相同）<br>
```{r}
#因為係數的指數就是勝率比，因此將係數分別取exp()
c(exp(0.0821), exp(0.0763))
```
...
**1.在`OfficeVisits`和`Narcotic`等於第一分位時<br>**
(1)模型預測PoorCare=1的機率為11.192%<br>
(2)勝率效果：<br>
當`OfficeVisits`每增加1，勝率會變成1.0856，增加了0.0856<br>
當`Narcotic`每增加1，勝率會變成1.0793，增加了0.0793<br>
(3)機率效果：<br>
當`OfficeVisits`每增加1，機率會變成0.12035；<br>
當`Narcotic`每增加1，機率會變成0.11973<br>


**2.在`OfficeVisits`和`Narcotic`等於第三分位時<br>**
(1)模型預測PoorCare=1的機率為28.947%<br>
(2)勝率效果：<br>
當`OfficeVisits`每增加1，勝率會變成1.0856，增加了0.0856；<br>
當`Narcotic`每增加1，勝率會變成1.0793，增加了0.0793<br>
(3)機率效果：<br>
當`OfficeVisits`每增加1，機率會變成0.30665；<br>
當`Narcotic`每增加1，機率會變成0.30541<br>

**3.從上面兩題可觀察到：<br>**
勝率的效果並不會改變，<br>機率的效果則會隨著變數本來的值（基準）而有不同邊際效果。<br>並且，在兩題中的`Narcotic`的勝率效果與機率效果都較`OfficeVisits`小。<br>
