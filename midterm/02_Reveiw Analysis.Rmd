...
```{r}
# 總商品數量的百分之二十
nrow(Prod)*0.2  # 6465.6

TwenProdValue = cumsum(Prod$revenue[1:6466])[6466]
TotalProdValue = sum(Prod$revenue)

TwenProdValue/TotalProdValue  # 0.7474123
```

```{r}
sort(Prod$revenue, decreasing = T)[6646]   # 399.96
Twenproduct = filter(Prod, revenue>=399.96)
# A=left_join(Twenproduct,OrdItm,by="product_id")
# B=left_join(A,Ord,by="order_id")
```



```{r}
Twenproduct = filter(Twenproduct, avgDeliveryDays>0)

ggplot(Twenproduct,aes(x=noPurchase,y=avgDeliveryDays,color=revenue))+
  geom_point()
```
